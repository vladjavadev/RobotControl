...
# Colors for different terrain difficulties
EASY_TERRAIN = (200, 255, 200)  # Light green
MEDIUM_TERRAIN = (255, 255, 150)  # Light yellow
HARD_TERRAIN = (255, 200, 150)  # Light orange
VERY_HARD_TERRAIN = (255, 150, 150)  # Light red

def get_terrain_color(weight):
    """Return color based on terrain weight"""
    if weight <= 1.0:
        return UNOCCUPIED
    elif weight <= 2.0:
        return EASY_TERRAIN
    elif weight <= 3.0:
        return MEDIUM_TERRAIN
    elif weight <= 4.0:
        return HARD_TERRAIN
    else:
        return VERY_HARD_TERRAIN
...
class Animation:
    ...
    def run_game(self, path=None):
        ...
        for event in pygame.event.get():
            ...
            # Set terrain difficulty using number keys
            keys = pygame.key.get_pressed()
            if any(keys):
                # Get mouse position for terrain modification
                (col, row) = pygame.mouse.get_pos()
                x = row // (self.height + self.margin)
                y = col // (self.width + self.margin)
                grid_cell = (x, y)
                
                # Only modify unoccupied cells
                if self.world.is_unoccupied(grid_cell):
                    if keys[pygame.K_1]:  # Normal terrain
                        self.world.weight_map[grid_cell] = 1.0
                    elif keys[pygame.K_2]:  # Slightly difficult
                        self.world.weight_map[grid_cell] = 2.0
                    elif keys[pygame.K_3]:  # Moderately difficult
                        self.world.weight_map[grid_cell] = 3.0
                    elif keys[pygame.K_4]:  # Very difficult
                        self.world.weight_map[grid_cell] = 4.0
                    elif keys[pygame.K_5]:  # Extremely difficult
                        self.world.weight_map[grid_cell] = 5.0
        ...
        for row in range(self.x_dim):
            for column in range(self.y_dim):
                ...
                # If cell is an obstacle, use obstacle color
                if self.world.occupancy_grid_map[row][column] == 255:
                    color = OBSTACLE
                else:
                    # Otherwise use terrain color based on weight
                    color = get_terrain_color(self.world.weight_map[cell_pos])
