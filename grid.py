...
class OccupancyGridMap:
    def __init__(self, x_dim, y_dim, exploration_setting='8N'):
        ...
        # the weight map (1.0 is default weight, higher values mean harder to traverse)
        self.weight_map = np.ones(self.map_extents, dtype=float)
...
class SLAM:
    ...
    def set_weight(self, pos: tuple[int, int], weight: float):
        ...
        if weight >= 1.0:
            x, y = pos
            self.slam_map.weight_map[x][y] = weight
            # Also update ground truth map to maintain consistency
    ...
    def c(self, u: tuple[int, int], v: tuple[int, int]) -> float:
        ...
        :param u: from vertex
        :param v: to vertex
        :return: weighted euclidean distance to traverse. inf if obstacle in path
        """
        if not self.slam_map.is_unoccupied(u) or not self.slam_map.is_unoccupied(v):
            ...
            u_weight = self.slam_map.weight_map[u[0]][u[1]]
            v_weight = self.slam_map.weight_map[v[0]][v[1]]
            
            # Use maximum weight between nodes to ensure difficult terrain is avoided
